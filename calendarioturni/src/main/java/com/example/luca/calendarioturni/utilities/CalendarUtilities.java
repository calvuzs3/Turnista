...
import com.varonesoft.luca.svgparser.SVG;
import com.varonesoft.luca.svgparser.SVGParser;
...
public class CalendarUtilities {
    ...
    public CalendarUtilities(Context context, boolean use24Hour) {
        ...
        calendars = new HashSet<AndroidCalendar>();
        selectedCalendars = new HashSet<AndroidCalendar>();
    }
     ...
     * @return
     * A Collection of DayEvents that are on the user's calendar
     */
    ...
    public String getFormattedTimeString(Context ctx, Event d) {
        ...
        if (d.getStart().before(todayDate) && d.getEnd().after(todayDate)) {
            return ctx.getResources().getString(R.string.word_now) + " - " + endString;
        } else {
    ...
    public static String getDateString(Context ctx, Event d, String formatString) {
        ...
        if (d.getStart().get(Calendar.DAY_OF_YEAR) == (now.get(Calendar.DAY_OF_YEAR) + 1) ||
            ...
            return ctx.getResources().getString(R.string.word_tomorrow);
        } else if (d.getStart().get(Calendar.DAY_OF_YEAR) == now.get(Calendar.DAY_OF_YEAR)) {
            return ctx.getResources().getString(R.string.word_today);
        } else {
